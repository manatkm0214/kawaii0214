    ...
    <div className="space-y-5 animate-fade-in">
          ...
          <tbody>
            {monthlyData.map(m => (
              <tr key={m.month} className="border-b border-slate-800/50">
                ...
                <td className="py-1 text-right">{m.savingRate}%</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex gap-3">
        <button
          onClick={downloadPDF}
          disabled={pdfLoading}
          className="flex-1 py-3 bg-red-700 hover:bg-red-600 disabled:opacity-50 rounded-xl text-sm font-medium transition-all"
        >
          {pdfLoading ? "生成中..." : "📄 PDFダウンロード"}
        </button>
        <button
          onClick={handlePrint}
          className="flex-1 py-3 bg-slate-700 hover:bg-slate-600 rounded-xl text-sm font-medium transition-all no-print"
        >
          🖨️ 印刷
        </button>
      </div>
